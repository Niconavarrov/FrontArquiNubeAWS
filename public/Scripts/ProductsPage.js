...
const SEARCH_ENDPOINT = ENDPOINTS.PRODUCTS_SEARCH;
...
window.searchProducts = async function(searchTerm) {
    try {
        ...
        const searchUrl = `${SEARCH_ENDPOINT}?q=${encodeURIComponent(searchTerm.trim())}`;
...
window.editProduct = async function(productId) {
    try {
        ...
        const response = await fetchWithToken(`${PRODUCTS_ENDPOINT}/${productId}`);
